        ...
        <v-flex xs2 pl-2>
          <v-menu v-model="menufilterdatestart" :close-on-content-click="false" :nudge-right="40" lazy
            ...
            <template v-slot:activator="{ on }">
              <v-text-field v-model="filterComputedDateFormattedStart" class="mr-2" label="Tanggal Awal" outline
                hide-details readonly v-on="on"
                @blur="date = deFormatedDate(filterComputedDateFormattedStart)"></v-text-field>
            </template>
            <v-date-picker no-title v-model="xdatestart" @input="menufilterdatestart = false"></v-date-picker>
          </v-menu>
        </v-flex>
        <v-flex xs2 pl-2>
          <v-menu v-model="menufilterdateend" :close-on-content-click="false" :nudge-right="40" lazy
            transition="scale-transition" offset-y full-width max-width="290px" min-width="290px">
            <template v-slot:activator="{ on }">
              <v-text-field v-model="filterComputedDateFormattedEnd" class="mr-2" label="Tanggal Akhir" outline
                hide-details readonly v-on="on"
                @blur="date = deFormatedDate(filterComputedDateFormattedEnd)"></v-text-field>
            </template>
            <v-date-picker no-title v-model="xdateend" @input="menufilterdateend = false"></v-date-picker>
          </v-menu>
        </v-flex>
        <v-flex xs2 pl-2>
    ...
    <v-card class="pa-2 mt-2">
      <v-layout row wrap class="scroll-container" style="max-height: 600px; overflow: auto">
        <v-flex xs12 pl-2 pr-2 pt-2 pb-2>
          <v-data-table :headers="headers" :items="xjurnals" :loading="isLoading" hide-actions class="elevation-1">
            <template slot="items" slot-scope="props">
              ...
              <td class="text-xs-center pa-2" v-bind:class="{ 'amber lighten-4': isSelected(props.item) }"
                @click="selectMe(props.item)">
                <v-icon small @click="openEditFormJurnal(props.item)">edit</v-icon>
                <v-icon small @click="openDeleteJurnal(props.item)">delete</v-icon>
              </td>
            </template>
          </v-data-table>
        </v-flex>
      ...
      <v-dialog v-model="dialogdeletealejurnal" max-width="30%">
        <v-card>
          ...
          <v-card-actions>
            ...
            <v-btn :disabled="xis_posted === 'Y'" color="primary" @click="closeDeleteAlertJurnal()">
              Ya
            </v-btn>
          </v-card-actions>
        </v-card>
  ...
  methods: {
    ...
    openEditFormJurnal(val) {
      ...
      if (val.JurnalTypeCode == "GOODRECEIVE") {
        ...
        this.$store.commit("jurnalumum/update_xdate", this.deFormatedDate(val.jurnalDate))
        ...
        this.$store.commit("jurnalumum/update_deskripsi", val.jurnalDescription)
    ...
    openDeleteJurnal(val) {
      ...
      this.msgalertjurnal = "Yakin, mau hapus jurnal [" + val.jurnalNo + " - " + val.jurnalTitle + "] ?"
