      ...
      <v-card>
        ...
        <v-card-text class="pt-0 pb-0">
          <v-form ref="formDetail" v-model="validationDetail" lazy-validtion>
            <v-layout wrap>
              <v-flex xs12>
                <v-text-field v-model="xDescriptionDetail" label="Deskripsi" :rules="descriptionRules"
                  required></v-text-field>
              </v-flex>
              <v-flex xs12>
                <v-text-field v-model="xAmountRequest" type="number" label="Jumlah" :min="1" :rules="amountRules"
                  required></v-text-field>
              </v-flex>
              <v-flex xs12>
                <v-text-field v-model="xEstimationPrice" label="Harga" type="number" :min="1" prefix="Rp "
        ...
        <v-card-actions>
          ...
          <v-btn v-if="act === 'new'" color="primary" dark @click="saveDetail()">Simpan</v-btn>
          <v-btn v-if="act === 'edit'" color="primary" dark @click="updateDetail()">Simpan Perubahan</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </v-layout>
