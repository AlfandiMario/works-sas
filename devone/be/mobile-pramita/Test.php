...
class Test extends MY_Controller
{
   ...
   function getdataflutter(){
    ...
    function getListingTest(){
		...
		// Validate branch ID
		$branchID = isset($prm['id']) ? intval($prm['id']) : 0;
		if ($branchID <= 0) {
			echo json_encode(array(
				"status" => "ERROR",
		...
		$date = empty($prm['date']) ? date('Y-m-d') : $prm['date'];
		...
		$currentPage = isset($prm['current_page']) ? intval($prm['current_page']) : 1;
		if ($currentPage < 1) {
			$currentPage = 1;
		}

		// Sanitize search string
		$search = isset($prm['search']) ? htmlspecialchars(trim($prm['search']), ENT_QUOTES, 'UTF-8') : '';

		// Validate categories array
		$categories = isset($prm['selected_category']) ? (array)$prm['selected_category'] : [0];
		$categories = array_map('intval', $categories); // Ensure all values are integers
		...
		if ($data_test) {
			$new_array = array();
			foreach ($data_test['rows'] as $v) {
				// Sanitize output data
				$schedule = array();
				$is_covid = 'N';
				$requirement = isset($v['requirement']) ? $v['requirement'] : array();
				$value_requirement = count($requirement) > 0 ? 'N' : 'X';

				if (!empty($v['schedule'])) {
					$schedules = array();
					foreach ($v['schedule'] as $v_sec) {
						$schedule = array(
							'nat_testid' => isset($v_child['Nat_TestID']) ? intval($v_child['Nat_TestID']) : 0,
							'testname' => isset($v_child['T_TestName']) ? htmlspecialchars($v_child['T_TestName'], ENT_QUOTES, 'UTF-8') : '',
							'date' => '',
							'time' => '',
							'datetime' => ''
						);
				...
				$variants = array();
				if (isset($v['T_PriceAmountCito']) && intval($v['T_PriceAmountCito']) > 0) {
					$variants = array(
						array(
							'name' => 'Reguler',
							'flag_cito' => 'N',
							'bruto' => intval($v['T_PriceAmount']),
							'price' => intval($v['T_PriceTotal']),
							'selected' => true
						),
						array(
							'name' => 'Cito',
							'flag_cito' => 'Y',
							'bruto' => intval($v['T_PriceAmountCito']),
							'price' => intval($v['T_PriceTotalCito']),
							'selected' => false
						)
					);
				}
				...
				$new_data = array(
					'moud_id' => isset($v['Ss_PriceMouM_MouID']) ? intval($v['Ss_PriceMouM_MouID']) : 0,
					'bruto' => isset($v['T_PriceAmount']) ? intval($v['T_PriceAmount']) : 0,
					'price' => isset($v['T_PriceTotal']) ? intval($v['T_PriceTotal']) : 0,
					'name' => isset($v['T_TestName']) ? htmlspecialchars($v['T_TestName'], ENT_QUOTES, 'UTF-8') : '',
					'description' => isset($v['description']) ? htmlspecialchars($v['description'] ?: '-', ENT_QUOTES, 'UTF-8') : '-',
					'type' => isset($v['is_packet']) && $v['is_packet'] == 'Y' ? 'PANEL' : 'TEST',
					'nat_test' => isset($v['nat_test']) ? json_decode($v['nat_test']) : null,
					'packet_id' => isset($v['packet_id']) ? intval($v['packet_id']) : 0,
	...
	function getListingAllPacket(){
        ...
        // Sanitize inputs
        $branch_id = intval($prm['id']); // Ensure integer
        $search = $this->db_onelite->escape_str(trim($prm['search'])); // Escape search string
        ...
        $sql = "SELECT Ss_PriceMouID as xid,
                ...
                JOIN t_packet ON T_PacketM_BranchID = ? AND T_PacketID = ss_price_mou_v2.packet_id
                WHERE 
                Ss_PriceMouM_BranchID = ? AND is_packet = 'Y' AND
                ss_price_mou_v2.T_TestName LIKE ?
                GROUP BY ss_price_mou_v2.T_TestID 
                ORDER BY T_PriceTotal ASC
                LIMIT ? OFFSET ?";

        $data_test = $this->db_onelite->query($sql, array(
            $branch_id,
            $branch_id,
            "%$search%",
            $number_limit,
			...
			foreach($data_test as $k => $v){
				...
				if($v['is_packet'] == 'Y'){
					foreach ($data_test[$k]['childs'] as $key => $value) {
						...
						$x = $this->db_onelite->query(
							"SELECT fn_fo_requirement_get(?) x",
							array($value->T_TestID)
						)->row();
						if ($x->x != null){
				...
				if($data_test[$k]['childs'] && count($data_test[$k]['childs']) > 0 ){
					foreach($data_test[$k]['childs'] as $k_child => $v_child){
						$sql = "SELECT *
								...
								WHERE
								T_SubcategoryTestT_TestID = ? AND 
								T_SubCategoryIsSchedule = 'Y' AND
								T_SubcategoryTestIsActive = 'Y'
								LIMIT 1";
						//if(intval($v['test_id']) == 3606)
						//echo $sql;
						$rst_child = $this->db_onelite->query($sql, array($v_child->T_TestID))->row_array();
						if($rst_child){
	...
	function getListingTestByCategory(){
        ...
        // Sanitize inputs
        $branch_id = intval($prm['id']); // Ensure integer
        $category_id = intval($prm['selected_category']); // Ensure integer
        $number_limit = 25;
        $number_offset = (intval($prm['current_page']) - 1) * $number_limit;
        ...
        $sql = "SELECT Ss_PriceMouID as xid,
                ...
                Nat_OLCategoryTestNat_OLCategoryID = ? AND Nat_OLCategoryTestIsActive = 'Y'
                ...
                M_TestFavoritM_UserID = ? AND M_TestFavoritIsActive = 'Y'
                WHERE 
                Ss_PriceMouM_BranchID = ? AND is_packet = 'N'
                GROUP BY ss_price_mou_v2.T_TestID 
                LIMIT ? OFFSET ?";
	...
	function getListingTestAll(){
        ...
        $sql = "SELECT Ss_PriceMouID as xid,
				...
				M_TestFavoritM_UserID = ? AND M_TestFavoritIsActive = 'Y'
				WHERE 
				Ss_PriceMouM_BranchID = ? AND 
				ss_price_mou_v2.T_TestName LIKE ? AND
				is_packet = 'N'
				GROUP BY ss_price_mou_v2.T_TestID 
				LIMIT ? OFFSET ?";
			//	echo $sql;
	...
	function getListingTestAllHS(){
        ...
        $sql = "SELECT Ss_PriceMouID as xid,
				...
				WHERE 
				Ss_PriceMouM_BranchID = ? AND 
				ss_price_mou_v2.T_TestName LIKE CONCAT('%',?,'%') AND
				is_packet = 'N'
				GROUP BY ss_price_mou_v2.T_TestID 
				limit $number_limit offset $number_offset ";
			//	echo $sql;
		$data_test = $this->db_onelite->query($sql,array($branch_id,$branch_id,$search))->result_array();
		//echo $this->db_onelite->last_query();
		if($data_test){
			foreach($data_test as $k => $v){
				$prices = $v['prices'];
	...
	function checkout(){
		...
		$sql = "SELECT COUNT(*) as x_count 
				...
				`Ss_PriceMouM_MouID` = ? 
				";
		$valid_data_mou = $this->db_onelite->query($sql, array(
			$prm['branch']['id'],
			$mouid
		))->row()->x_count;
		...
		if(count($not_avalilable_schedule) == 0 && $valid_data_mou  > 0){
			...
			foreach($cart as $kxcart => $vxcart){
				...
				$sql = "INSERT INTO t_order(
							...
							VALUES(?,?,?,?,?,?,?,?,?,?,?,NOW())";
				$order = $this->db_onelite->query($sql, 
							array(
								$trx_id,
				...
				if($order){
					...
					foreach($vxcart['order'] as $kxorder => $vxorder){
						...
						$sql = "INSERT INTO t_orderdetail(
									...
									VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,NOW())";
						$orderdetail = $this->db_onelite->query($sql, 
									array(
										$order_id,
										$packet_name,
										$packet_id,
						...
						if(count($vxorder['schedule']) > 0){
							foreach($vxorder['schedule'] as $kxschedule => $vxschedule){
								$sql = "INSERT INTO t_orderschedule(
											...
											VALUES(?,?,?,?,?,?,?,NOW())";
								$schedule = $this->db_onelite->query($sql, 
											array(
												$order_id,
												$vxschedule['test_id'],
			...
			if($prm['order_hs']){
				...
				{
					$sql = "SELECT get_username_trx('{$row_t_numbering['numbering']}') as xname";
					$qry = $this->db_onelite->query($sql);
					//echo $this->db_onelite->last_query();
					$get_order_name = $qry->row_array();
					$order_name_hs = $get_order_name['xname'];
					//print_r($get_order_name);
			...
			if($valid_data_mou > 0){
				...
				echo json_encode( array(
					"status" => "ERR",
					"message" => "Maaf ada kendala sistem, silahkan lakukan order ulang"
				));
			}
		}
	...
	function checkout_xxx(){
		...
		$cart = $prm['cart'];
		$mouid = 0;
		$xdistance = explode("FHM", isset($prm['code_dist'])?$prm['code_dist']:"0FHM");
		$distance = $xdistance[0];
		foreach($cart as $kcart => $vcart){
			foreach($vcart['order'] as $korder => $vorder){
				$mouid = $vorder['mou_id'];
				$schedules = $vorder['schedule'];
		...
		if(count($not_avalilable_schedule) == 0 && $valid_data_mou  > 0){
			$sql = "SELECT fn_numbering('TRO') as numbering";
			$isHS = 'N';
			$trx_distance = $prm['branch']['distance'];
			if($prm['order_hs']){
				$isHS = 'Y';
				$xdistance = explode("FHM", isset($prm['code_dist'])?$prm['code_dist']:'0FHM');
				$trx_distance = $xdistance[0];
			}
			$row_t_numbering = $this->db_onelite->query($sql)->row_array();
			$sql = "INSERT INTO t_transaction(
						T_TransactionAppVersion,
			...
			$app_version = '3.0'; // Hardcoded value
			$numbering = $this->db_onelite->escape_str($row_t_numbering['numbering']);
			$branch_id = intval($prm['branch']['id']); // Ensure integer
			$mou_id = intval($mouid); // Ensure integer
			$distance = floatval($trx_distance); // Ensure numeric
			$user_id = intval($user['M_UserID']); // Ensure integer
			$is_hs = in_array($isHS, ['Y', 'N']) ? $isHS : 'N'; // Validate Y/N

			// Use parameterized query
			$trx = $this->db_onelite->query($sql, array(
				$app_version,
				$numbering,
				$branch_id,
				$mou_id, 
			...
			foreach($cart as $kxcart => $vxcart){
				...
				// Validate and sanitize inputs before inserting
				$trx_id = intval($trx_id); // Ensure integer
				$numbering = $this->db_onelite->escape_str($row_numbering['numbering']); // Escape string
				$qr_code = $this->db_onelite->escape_str($row_qrCode['qrCodeCode']); // Escape string
				$branch_id = intval($prm['branch']['id']); // Ensure integer
				$patient_id = intval($vxcart['patient']['M_PatientID']); // Ensure integer
				$purpose_id = intval($purpose_id); // Ensure integer
				$ehac_fee = floatval($ehac_fee); // Ensure numeric
				$is_klinisi = in_array($is_klinisi, ['Y', 'N']) ? $is_klinisi : 'N'; // Validate Y/N
				$datevisit = date('Y-m-d', strtotime($datevisit)); // Validate date format
				$timevisit = date('H:i:s', strtotime($timevisit)); // Validate time format
				$user_id = intval($user['M_UserID']); // Ensure integer
				...
				$order = $this->db_onelite->query($sql, array(
					$trx_id,
					$numbering,
					$qr_code, 
					$branch_id,
				...
				if($order){
					...
					foreach($vxcart['order'] as $kxorder => $vxorder){
						...
						$sql = "SELECT T_PriceTotal as price
								FROM ss_price_mou_v2
								WHERE
								Ss_PriceMouM_MouID = ? AND
								Ss_PriceMouM_BranchID = ? AND
								T_TestID = ? AND
								T_PriceIsCito = ? AND
								is_packet = ?";
						...
						if(!$query){
							$sql = "UPDATE t_transaction SET T_TransactionIsActive = 'N'
									WHERE
									T_TransactionID = ?";
							$query = $this->db_onelite->query($sq,array($trx_id));
							echo json_encode( array(
								"status" => "ERR",
								"message" => 'Penyimpanan gagal, silahkan transaksi ulang'
							));
			...
			if($prm['order_hs']){
				$xdistance = explode("FHM", $prm['code_dist']);
				$distance = $xdistance[0];
				$sql = "SELECT *
					FROM hs_price
					WHERE
						HS_PriceM_BranchID = ? AND 
						( ? > HS_PriceMinDistance AND ? <= HS_PriceMaxDistance ) AND 
						HS_PriceIsActive = 'Y'";
				$qry = $this->db_onelite->query($sql, array($prm['branch']['id'],$distance,$distance));
				//echo $this->db_onelite->last_query();
				if(!$qry){
					$sql = "UPDATE t_transaction SET T_TransactionIsActive = 'N'
							WHERE
				...
				$hs_cost = $qry->row()->HS_PriceAmount;
				

				if($prm['selected_hs_address']['id'] && intval($hs_cost) > 0)
				{
					$sql = "SELECT get_username_trx('{$row_t_numbering['numbering']}') as xname";
					$qry = $this->db_onelite->query($sql);
					//echo $this->db_onelite->last_query();
					...
					$trx = $this->db_onelite->query($sql, 
								array(
									intval($trx_id),
									$hs_cost,
									intval($prm['selected_hs_address']['id']),
									intval($user['M_UserID']),
									$order_name_hs,
									$user['M_UserUsername']
								)
							);
...
//echo date("l jS \of F Y h:i:s A");
	...
	function getOrderTotal(){
		...
		// Validate inputs
		$transaction_id = isset($prm['id']) ? intval($prm['id']) : 0; // Ensure integer
		$user_id = isset($user['M_UserID']) ? intval($user['M_UserID']) : 0; // Ensure integer
		...
		$sql = "SELECT T_TransactionID as id,
				...
				WHERE T_TransactionID = ? AND T_TransactionUserID = ?";

		$row = $this->db_onelite->query($sql, array($transaction_id, $user_id))->row_array();
		//echo $this->db_onelite->last_query();
		//print_r($row);
		if($row){
			...
			if($row['order_hs'] == 'Y'){
				...
				$sql = "UPDATE t_transactionHS SET t_transactionHSCostMinOrder = ?, t_transactionHSCostOther = ? WHERE t_transactionHST_TransactionID = ?";
				$update_costminorder = $this->db_onelite->query($sql, array($selisih,$number_costother,$prm['id']));
			...
			$sql = "SELECT T_OrderID as id,
				...
				WHERE
					T_OrderT_TransactionID = ? 
				GROUP BY T_OrderID";
			$row['patients'] = $this->db_onelite->query($sql, array($transaction_id))->result_array();
			//echo $this->db_onelite->last_query();
			//print_r($row);
			$ehac = 0;
			$ehac_fee = 0 ;
		...
		if($dt_payments){
			foreach($dt_payments as $k => $v){
				...
				$parent_id = isset($v['parent_id']) ? intval($v['parent_id']) : 0;
				$dt_payments_childs = $this->db_onelite->query($sql, array($parent_id))->result_array();
				//echo $this->db_onelite->last_query();
	...
	function pay(){
		...
		// Validate and sanitize inputs
		$order_number = $this->db_onelite->escape_str($prm['order']['order_number']); // Escape string
		$user_id = intval($user['M_UserID']); // Ensure integer

		// Verify inputs are valid
		if (empty($order_number) || $user_id <= 0) {
			echo json_encode([
				"status" => "ERR", 
				"message" => "Invalid input parameters"
			]);
		...
		// Use parameterized query with validated inputs
		$sql = "SELECT T_TransactionID, T_TransactionTotal, configFee
				FROM t_transaction
				JOIN config ON configIsActive = 'Y'
				WHERE T_TransactionNumbering = ? AND T_TransactionUserID = ?
				LIMIT 1";
		$query = $this->db_onelite->query($sql, array($order_number, $user_id));
		if(!$query){
			echo json_encode( array(
				"status" => "Err",
				"message" => "Nomor transaksi tidak ditemukan, harap login ulang",
				"inp" => $inp
			));
			exit;
		}
		...
		$db_transaction_id = $row_order['T_TransactionID'];
		$db_total_order = $row_order['T_TransactionTotal'];
		$db_fee_original = $row_order['configFee'];

		$sql = "SELECT COUNT(*) as xcount
				FROM t_transactionHS
				WHERE
				t_transactionHST_TransactionID = ? AND t_transactionHSIsActive = 'Y'
				LIMIT 1";
		$query = $this->db_onelite->query($sql,array($db_transaction_id));

		$xcount_hs = $query->row()->xcount;
		
		if($xcount_hs > 0){
			$sql = "SELECT IFNULL(t_transactionHSCost,0) as t_transactionHSCost,
						IFNULL(t_transactionHSCostOther,0) as t_transactionHSCostOther,
						IFNULL(t_transactionHSCostMinOrder,0) as t_transactionHSCostMinOrder
					FROM t_transactionHS
					WHERE
					t_transactionHST_TransactionID = ? AND t_transactionHSIsActive = 'Y'
					LIMIT 1";
			$query = $this->db_onelite->query($sql,array($db_transaction_id));
			//echo $this->db_onelite->last_query();
			$row_order_hs = $query->row_array();
			//print_r($row_order_hs);
			$db_total_order = intval($db_total_order) + intval($row_order_hs['t_transactionHSCost']) + intval($row_order_hs['t_transactionHSCostOther']) + intval($row_order_hs['t_transactionHSCostMinOrder']);
		}

		$db_fee = 0;

		if(intval($prm['payment']['type_id']) == 1){
			$db_fee = round(( 1/100 * $db_total_order ) + $db_fee_original);
        }
		else if(intval($prm['payment']['type_id']) != 5){
            $db_fee = $db_fee_original;
		}
		...
		if($prm['payment']['code'] != 'PO' && $prm['payment']['code'] != 'PT'){
			...
			if($row_pg){
				...
				$sql = "UPDATE t_transaction SET T_TransactionFee = ?
						WHERE
							T_TransactionNumbering = ?";
				$this->db_onelite->query($sql,array($db_fee,$prm['order']['order_number']));
				...
				if(!$prm['paywith_point'] || ($prm['paywith_point'] && intval($points) < intval($prm['order']['number_total_fee'])) ){
					...
					$item_details = array(
						array("name" => 'Total Pemeriksaan', "quantity" => 1, "price" => $db_total_order),
						array("name" => 'Biaya Layanan', "quantity" => 1, "price" => $db_fee)
					);
					...
					$db_total_fee = $db_total_order + $db_fee;

					$inp = array(
						//"paymentAmount" => $prm['order']['number_total_fee'],
						"paymentAmount" => $db_total_fee,
