...
class Samplingcall extends MY_Controller
{
	...
	function getsampletypes()
	{
		...
		$sql = "SELECT T_OrderDetailID as id, 
				...
				T_TestID as test_id,
				T_TestNat_TestID as nat_test_id,
				T_BahanName,
				...
				IF(Group_ResultName = 'Rontgen',IF(ISNULL(T_SamplingSoFormExpose),'N','Y'),'Y') as fill_form,
		...
		if ($rows) {
			foreach ($rows['sampletypes'] as $k => $v) {
				...
				$sql = "SELECT *
						FROM m_eksposi
						WHERE
						M_EksposiNat_TestID = {$v['nat_test_id']} AND M_EksposiIsActive = 'Y'
						ORDER BY M_EksposiID DESC 
						LIMIT 1";
				$data_xposi = $this->db_onedev->query($sql)->row_array();
				if ($data_xposi) {
					if ($v['form_kv'] == '') {
						$rows['sampletypes'][$k]['form_kv'] = $data_xposi['M_EksposiKV'];
					}
					if ($v['form_ma'] == '') {
						$rows['sampletypes'][$k]['form_ma'] = $data_xposi['M_EksposiMa'];
					}
					if ($v['form_second'] == '') {
						$rows['sampletypes'][$k]['form_second'] = $data_xposi['M_EksposiS'];
					}
					if ($v['form_expose'] == '') {
						$rows['sampletypes'][$k]['form_expose'] = $data_xposi['M_EksposiExpose'];
					}
				}
			}
		}
	...
	function cobacall()
	{
		$sql = "SELECT T_OrderHeaderQueue AS queueNumber ,
				...
				JOIN t_order_location ON T_OrderHeaderID = T_OrderLocationT_OrderHeaderID
				JOIN m_location ON T_OrderLocationM_LocationID = M_LocationID
				AND T_OrderLocationT_SampleStationID = 2
				WHERE  T_OrderHeaderID=131560";
		$location = $this->db_onedev->query($sql)->row_array();
		$locationID = $location['locationID'];
		$locationName = $location['locationName'];
		$queueNumber = $location['queueNumber'];
		print_r($location);
		print_r($location['locationID']);
		$splitedLocationName = explode(" ", $locationName);
		$locationName = implode("+", $splitedLocationName);
		try {
			//code...
			file_get_contents("http://" . $this->IP_SOCKET_IO . ":9099/broadcast/call.sm.$locationID.$queueNumber.$locationName");
		} catch (Exception $e) {
			print_r($e);
		}
	}
...
// t_orderheader.*,
// tDATE_FORMAT(M_PatientDOB,'%d-%m-%Y') as dob,
// m_patient.*,
// tIFNULL(M_PatientPhoto,'') as M_PatientPhotoThumb,
// M_SexName, 
// M_TitleName, 
// CONCAT(M_TitleName,' ',M_PatientName) as patient_fullname, tM_CompanyName,
// fn_sampling_queue_status_name(T_OrderHeaderID,T_SampleStationID) as status, 
// DATE_FORMAT(M_PatientDOB,'%d-%m-%Y') as patient_dob,
// fn_sampling_queue_status_id(T_OrderHeaderID,T_SampleStationID)  as statusid,
// T_SampleStationID, 
...
// JOIN t_sampletype ON T_TestT_SampleTypeID = T_SampleTypeID
// JOIN t_bahan ON T_SampleTypeT_BahanID = T_BahanID
// JOIN t_samplestation ON T_BahanT_SampleStationID = T_SampleStationID AND T_SampleStationID = 2 AND T_SampleStationIsNonLab = 'RADIODIAGNOSTIC'
// JOIN last_status ON Last_StatusT_OrderHeaderID = T_OrderHeaderID
// LEFT JOIN t_samplingso ON T_SamplingSoT_OrderHeaderID = T_OrderHeaderID AND 
// T_SamplingSoT_TestID = T_TestID AND 
// T_SamplingSoT_SampleStationID = T_SampleStationID AND
// T_SamplingSoIsActive = 'Y'
// WHERE T_OrderHeaderIsActive = 'Y' AND ( DATE(T_OrderHeaderAddonIsComingDate) = '2023-02-21' OR DATE(T_OrderHeaderDate) = '2023-02-21' )  AND ( ISNULL(T_SamplingSoID) OR T_SamplingSoFlag = 'P' OR T_SamplingSoFlag = 'X')
// tGROUP BY T_OrderHeaderID
// HAVING last_status_fo IN (3,5)
