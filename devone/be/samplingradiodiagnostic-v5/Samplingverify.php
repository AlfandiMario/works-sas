...
class Samplingverify extends MY_Controller
{
   ...
   public function search()
   {
	  ...
	  if ($nolab != "") {
		...
		$sql_where = "WHERE T_OrderHeaderIsActive = 'Y' {$where_status} {$filter_search}";
		...
		$sql = 	"SELECT T_OrderHeaderID,
					T_OrderHeaderLabNumberExt, T_TestName as T_SampleTypeName,T_OrderHeaderLabNumber,
						...
						'' as requirements,
						IF(Group_ResultName = 'Rontgen',IF(ISNULL(T_SamplingSoFilmID),'N','Y'),'Y') as fill_form,
						IF(Group_ResultName = 'Rontgen','xray',Group_ResultName) as type,
						IFNULL(T_SamplingSoFilmType,'N') as selected_filmtype,
						IFNULL(T_SamplingSoFilmAmount,0) as amount_film
				FROM t_orderheader	
				...
				ORDER BY T_OrderHeaderIsCito DESC, T_SamplingSoDoneDate ASC, T_SamplingSoDoneTime ASC, T_OrderHeaderID ASC
				";
		//echo $sql;
		...
		if($rows){
			foreach($rows as $k => $v){
				$zxprm = array();
				if($v['selected_filmtype'] == 'N')
					$rows[$k]['selected_filmtype'] = array('id'=>$v['selected_filmtype'],'name'=>'Tanpa Film');
				if($v['selected_filmtype'] == 'S')
					$rows[$k]['selected_filmtype'] = array('id'=>$v['selected_filmtype'],'name'=>'Film Kecil');
				if($v['selected_filmtype'] == 'M')
					$rows[$k]['selected_filmtype'] = array('id'=>$v['selected_filmtype'],'name'=>'Film Sedang');
				if($v['selected_filmtype'] == 'XL')
					$rows[$k]['selected_filmtype'] = array('id'=>$v['selected_filmtype'],'name'=>'FIlm Besar');
	...
	function doaction(){
		...
		if($prm['act'] == 'verify'){
			...
			if($prm['sample']['type'] == 'xray'){
				...
				$T_SamplingSoFilmID = 0;
				$sql = "SELECT * FROM t_samplingso_film WHERE T_SamplingSoFilmT_SamplingSoID = {$prm['sample']['T_SamplingSoID']} AND T_SamplingSoFilmIsActive = 'Y' LIMIT 1";
				//echo $sql;
				$row_x = $this->db_onedev->query($sql)->row_array();
				if($row_x){
					$T_SamplingSoFilmID = $row_x['T_SamplingSoFilmID'];
				}
				if($T_SamplingSoFilmID == 0){
					$sql = "INSERT INTO t_samplingso_film (
								T_SamplingSoFilmT_SamplingSoID,
								T_SamplingSoFilmType,
								T_SamplingSoFilmAmount,
								T_SamplingSoFilmCreated,
								T_SamplingSoFilmUserID
							)
							VALUES(
								{$prm['sample']['T_SamplingSoID']},
								'{$prm['sample']['selected_filmtype']['id']}',
								{$prm['sample']['amount_film']},
								NOW(),
								{$userid}
							)";
					$this->db_onedev->query($sql);
				}else{
					$sql = "UPDATE t_samplingso_film SET
								T_SamplingSoFilmT_SamplingSoID = {$prm['sample']['T_SamplingSoID']},
								T_SamplingSoFilmType = '{$prm['sample']['selected_filmtype']['id']}',
								T_SamplingSoFilmAmount = {$prm['sample']['amount_film']},
								T_SamplingSoFilmUserID = {$userid}
							WHERE
								T_SamplingSoFilmID = {$T_SamplingSoFilmID}
							)";
					$this->db_onedev->query($sql);
				}
